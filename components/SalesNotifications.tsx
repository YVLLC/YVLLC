...
const TIMEAGO = [
  "just now","10 seconds ago","a minute ago","2 minutes ago","5 minutes ago","8 minutes ago","12 minutes ago","18 minutes ago",
  "20 minutes ago","30 minutes ago","45 minutes ago","an hour ago","2 hours ago","3 hours ago","6 hours ago","10 hours ago",
  "12 hours ago","18 hours ago","22 hours ago","a day ago","2 days ago","3 days ago","5 days ago","a week ago","8 days ago",
  "10 days ago","12 days ago","2 weeks ago","20 days ago","3 weeks ago","a month ago","5 weeks ago"
];
...
function makeNotifications(howMany = 50): NotificationData[] {
  ...
  for (let i = 0; i < howMany; i++) {
    ...
    all.push({ location: city, platform: svc.platform, type: svc.type, amount: amt, timeAgo: time });
  }
  return shuffle(all);
}

function getServiceLabel(platform: string, type: string, amount: number) {
  const svc = SERVICES.find(s => s.platform === platform && s.type === type);
  return svc ? svc.label(amount) : `${amount.toLocaleString()} ${platform} ${type}`;
}

function getServiceIcon(platform: string) {
  switch (platform) {
    case "Instagram": return <Instagram className="text-[#E1306C]" size={19} />;
    case "TikTok": return <Music2 className="text-[#25F4EE]" size={19} />;
    case "YouTube": return <Youtube className="text-[#FF0000]" size={19} />;
    default: return <span />;
  }
}

const NOTIFY_INTERVAL = 180000;
...
export default function SalesNotifications() {
  ...
  useEffect(() => {
    ...
    try {
      const stored = window.sessionStorage.getItem("sales_notifs_data");
      notifications = stored ? JSON.parse(stored) : makeNotifications(50);
    ...
    const storedIdx = window.sessionStorage.getItem("sales_notifs_idx");
    const startIdx = storedIdx ? parseInt(storedIdx, 10) : 0;
    setIdx(startIdx);

    const now = Date.now();
    const lastShown = parseInt(window.sessionStorage.getItem("sales_notifs_last_time") || "0", 10);
  ...
  const n = notifs[idx];
  const icon = getServiceIcon(n.platform);
  const label = getServiceLabel(n.platform, n.type, n.amount);
    ...
    <>
      <div
        className={`
          ...
          w-[300px] sm:w-[340px]
          bg-white/80 backdrop-blur-xl border border-[#CFE4FF]/70
          shadow-[0_8px_40px_rgba(0,123,255,0.18)] rounded-3xl
          flex items-start gap-4 px-5 py-4 transition-all duration-700
          ${visible ? "opacity-100 translate-y-0 pointer-events-auto animate-notify-in" : "opacity-0 translate-y-8 pointer-events-none"}
        `}
        style={{ fontFamily: "inherit", willChange: "opacity, transform" }}
        aria-live="polite"
      >
        <div className="flex items-center justify-center w-12 h-12 rounded-2xl bg-gradient-to-br from-[#E6F0FF] to-white border border-[#CFE4FF]/70 shadow-inner">
          <div className="scale-[1.15] opacity-90">{icon}</div>
        </div>

        <div className="flex flex-col">
          <div className="flex items-center gap-2 mb-0.5">
            <span className="text-[13px] font-bold text-[#007BFF] tracking-tight">Recent Order</span>
            <span className="inline-block w-2 h-2 bg-[#22C55E] rounded-full animate-pulse" />
          </div>

          <div className="text-[12px] text-[#444] font-medium leading-tight">{n.location}</div>

          <div className="text-sm font-extrabold text-[#111] mt-1 tracking-tight">{label}</div>

          <div className="text-[11px] text-[#777] mt-1">{n.timeAgo}</div>
        </div>
      </div>
